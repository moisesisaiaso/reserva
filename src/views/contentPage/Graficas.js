...
import { chartExample1, chartExample2, chartExample3, chartExample4 } from "variables/charts.js"; // Ensure these variables are defined correctly
import Header from "components/Headers/Header";

// Default chart configuration to avoid errors if variables are undefined
const defaultChart = {
    data: {
        labels: [],
        datasets: [],
...
const Graficas = () => {
    ...
    const procesarDatosParaGrafico = () => {
        ...
        const reservasPorMes = meses.map(
            (_, index) =>
                reservas.filter((reserva) => new Date(reserva.fecha_reserva).getMonth() === index)
                    .length
        );
        return {
            labels: meses,
